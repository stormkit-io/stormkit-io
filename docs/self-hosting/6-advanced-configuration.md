...
| Variable                      | Default | Description                                                                                                                                                                 |
| ----------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `STORMKIT_HTTP_READ_TIMEOUT`  | `30s`   | Maximum time to read an entire request, including the body. Requests with a non-zero `Content-Length` bypass this timeout to allow large file uploads.                      |
| `STORMKIT_HTTP_IDLE_TIMEOUT`  | `60s`   | Maximum time to wait for the next request on a keep-alive connection.                                                                                                       |
| `STORMKIT_HTTP_PROXY_TIMEOUT` | `30s`   | Maximum time the outgoing proxy client waits for a response from an upstream server. Set to `0` to disable. Streaming requests (file uploads) always bypass this timeout. |
