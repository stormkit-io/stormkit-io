...
To enable image optimization, build with the `imageopt` build tag:

```bash
go build -tags=imageopt ./src/ce/hosting
```

### Building without Image Optimization (Default)

When building without the `imageopt` tag, image optimization is disabled and no additional system dependencies are required:
...
For Docker deployments, include the system dependencies in your Dockerfile when building with image optimization:
...
RUN apk add --no-cache vips-dev pkgconf gcc musl-dev

# Build with image optimization enabled
RUN go build -tags=imageopt -o /app/hosting ./src/ce/hosting

# Runtime image
FROM alpine:latest
RUN apk add --no-cache vips
COPY --from=builder /app/hosting /app/hosting
```
