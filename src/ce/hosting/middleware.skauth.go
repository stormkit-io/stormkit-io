...
func (m *skAuthMiddleware) handleMagicLinkVerify() (*shttp.Response, error) {
	...
	if redirect, _ := data["redirect"].(string); redirect != "" {
		if res := m.crossOriginCookieResponse(redirect, successURL, token); res != nil {
			return res, nil
		}
...
func (m *skAuthMiddleware) crossOriginCookieResponse(origin, successURL, token string) *shttp.Response {
	if !m.req.Host.Config.SKAuth.SessionInCookie() {
		return nil
	}

	return m.deliverSession(token, origin+successURL)
}
