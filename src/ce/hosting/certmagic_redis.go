...
// NOTE: This file has been copied from https://github.com/pberkel/caddy-storage-redis
// and modified to fit Stormkit's needs.
package hosting
...
type RedisStorage struct {
	// KeyPrefix A string prefix that is appended to Redis keys. Default: "le_"
...
// NewRedisStorage creates a new RedisStorage struct.
// SetClient must be called afterwards to set the Redis client.
...
// Load retrieves the value for the given key from Redis storage.
...
// Delete removes the given key from Redis storage.
...
// List lists all keys stored in Redis under the given directory.
...
// Stat retrieves metadata about the given key from Redis storage.
...
// Lock obtains a distributed lock for the given name using Redis.
...
// Unlock releases the distributed lock for the given name using Redis.
...
// Repair traverses the entire Redis storage and repairs the directory structure indexes.
